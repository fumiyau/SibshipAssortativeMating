...
ggsave(height=6,width=8,dpi=200, filename="Figures/4.Decomp/DecompFemaleAR5-CFAR-4cat-Hazard.pdf",  family = "Helvetica")
...
ggplot(df2, aes(x=SibStrF, y=Cont, fill=AR,group=AR)) + 
  ...
  theme(legend.title=element_blank())+ylim(-0.35,0.1)+theme(axis.text.x=element_text(angle=60,vjust=1,hjust=1))
ggsave(height=6,width=8,dpi=200, filename="Figures/4.Decomp/DecompMaleAR5-CFAR-4cat-Hazard.pdf",  family = "Helvetica")


######################################################################
# Rate Model
######################################################################
df1 <- read_dta("1_Data/2_HM_Model/Rate-Model/02_Output/02_HM/Sib4Cat/DecompPSTMFR.dta") %>% 
  dplyr::select(SibStrF, SibStrM, ContAR_F, ContAR_M) %>% 
  gather( "Female", "Cont", 3:4) %>% 
  mutate(Female=case_when(
    Female == "ContAR_F" ~ 1,
...
df2 <- read_dta("1_Data/2_HM_Model/Rate-Model/02_Output/02_HM/Sib4Cat/DecompPSTMFR.dta") %>% 
  dplyr::select(SibStrF, SibStrM, ContFA_F, ContFA_M) %>% 
  gather( "Female", "Cont", 3:4) %>% 
  mutate(Female=case_when(
    Female == "ContFA_F" ~ 1,
...
ggplot(df1, aes(x=SibStrM, y=Cont, fill=AR,group=AR)) + 
  geom_bar(stat="identity")+xlab("")+ylab("")+theme_few() +facet_wrap(~SibStrF)+scale_fill_brewer(palette="Paired") +
  theme(legend.title=element_blank())+ylim(-0.35,0.1)+theme(axis.text.x=element_text(angle=60,vjust=1,hjust=1))
ggsave(height=6,width=8,dpi=200, filename="Figures/4.Decomp/DecompFemaleAR5-CFAR-4cat-Rate.pdf",  family = "Helvetica")

ggplot(df2, aes(x=SibStrF, y=Cont, fill=AR,group=AR)) + 
  geom_bar(stat="identity")+xlab("")+ylab("")+theme_few() +facet_wrap(~SibStrM) + scale_fill_brewer(palette="Paired") +
  theme(legend.title=element_blank())+ylim(-0.35,0.1)+theme(axis.text.x=element_text(angle=60,vjust=1,hjust=1))
ggsave(height=6,width=8,dpi=200, filename="Figures/4.Decomp/DecompMaleAR5-CFAR-4cat-Rate.pdf",  family = "Helvetica")
