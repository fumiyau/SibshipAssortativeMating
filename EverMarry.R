...
## CF value ##
#### #### ####
df2 <- read_dta("1_Data/2_HM_Model/Hazard-Model/02_Output/02_HM/Sib4Cat/PSTFMH_PSTFMR_CF.dta") %>%
  dplyr::select(-PSTFMH_CF_F, PSTFMH_CF_M) %>% 
  pivot_longer(
    cols = starts_with("PSTFMR_"),
    names_to = "sex",
    names_prefix = "PSTFMR_CF_",
    values_to = "rate",
    values_drop_na = TRUE
  ) %>% 
...
ggsave(height=6,width=9,dpi=200, filename="Figures/6.Comparison/PSTFMR-ObsCF-Female-4cat-Hazard.pdf",  family = "Helvetica")

df %>% filter(sex=="M") %>% 
  ggplot(aes(x=SibStrF, y=rate, fill=Year5yRnd,group=Year5yRnd)) +facet_wrap(~SibStrM)+
  geom_bar(stat="identity", position="dodge")+xlab("")+ylab("")+theme_few() +scale_fill_brewer(palette="Blues") +
  theme(legend.title=element_blank(), legend.position = "bottom")+ylim(0,1)+theme(axis.text.x=element_text(angle=20,vjust=1,hjust=1))
ggsave(height=6,width=9,dpi=200, filename="Figures/6.Comparison/PSTFMR-ObsCF-Male-4cat-Hazard.pdf",  family = "Helvetica")
...
df1 <- read_dta("1_Data/2_HM_Model/Rate-Model/02_Output/02_HM/Sib4Cat/PSTFMR_obs.dta") %>%
  filter(Year5yRnd == 1 | Year5yRnd == 6) %>%
  pivot_longer(
    cols = ends_with("_obs"),
    names_to = "type",
...
## CF value ##
#### #### ####
df2 <- read_dta("1_Data/2_HM_Model/Rate-Model/02_Output/02_HM/Sib4Cat/PSTFMR_CF.dta") %>%
  pivot_longer(
    cols = starts_with("PSTFMR_"),
    names_to = "sex",
    names_prefix = "PSTFMR_CF_",
...
df %>% filter(sex=="F") %>% 
  ...
  geom_bar(stat="identity", position="dodge")+xlab("")+ylab("")+theme_few() +scale_fill_brewer(palette="Blues") +
  theme(legend.title=element_blank(), legend.position = "bottom")+ylim(0,1)+theme(axis.text.x=element_text(angle=20,vjust=1,hjust=1))
ggsave(height=6,width=9,dpi=200, filename="Figures/6.Comparison/PSTFMR-ObsCF-Female-4cat-Rate.pdf",  family = "Helvetica")

df %>% filter(sex=="M") %>% 
  ggplot(aes(x=SibStrF, y=rate, fill=Year5yRnd,group=Year5yRnd)) +facet_wrap(~SibStrM)+
  geom_bar(stat="identity", position="dodge")+xlab("")+ylab("")+theme_few() +scale_fill_brewer(palette="Blues") +
  theme(legend.title=element_blank(), legend.position = "bottom")+ylim(0,1)+theme(axis.text.x=element_text(angle=20,vjust=1,hjust=1))
ggsave(height=6,width=9,dpi=200, filename="Figures/6.Comparison/PSTFMR-ObsCF-Male-4cat-Rate.pdf",  family = "Helvetica")
